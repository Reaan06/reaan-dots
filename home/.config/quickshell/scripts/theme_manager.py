...
# spicetify compiles the colours into the Spotify client, so writing color.ini
# has no effect until `spicetify apply` runs (`./setup spotify`).
SPICETIFY_DIR = os.path.join(XDG_CONFIG_HOME, "spicetify")
...
def build_spicetify_colors(colors):
    ...
    lines = ["; GENERATED by scripts/theme_manager.py on every palette change.",
             "; Edits here are lost at the next switch, and do not reach",
             "; Spotify at all until `./setup spotify` has run.",
             "",
             "[base]"]
...
def write_spicetify_colors(colors):
    ...
    Guarded on spicetify rather than spotify. It does not touch
    config-xpui.ini (set once by hand) and does not run `spicetify apply`,
    which rewrites /opt/spotify and restarts the client.
    """
