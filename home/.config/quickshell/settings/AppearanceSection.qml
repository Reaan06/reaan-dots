...
SettingsSection {
    ...
    Component.onCompleted: {
        ...
        // Loaded up front so the rules card has rows as soon as it expands.
    ...
    ColumnLayout {
        ...
        SettingTiles {
            label: Tr.t("Effect")
            reading: {
                const entry = WallpaperService.transitions.find(
                    row => row.id === SettingsService.wallpaperTransition)
                return entry ? Tr.t(entry.label) : ""
            }
        ...
        SettingTiles {
            label: Tr.t("Scene")
            reading: {
                const entry = ThemeService.greetings.find(
                    row => row.id === SettingsService.greeting)
                return entry ? Tr.t(entry.label) : ""
            }
    ...
    ColumnLayout {
        ...
        SettingRow {
            label: Tr.t("Glass")
            reading: SettingsService.windowGlass ? Tr.t("Frosted") : Tr.t("Off")
        ...
        // ── WINDOW RULES ────────────────────────────────────────────────────
        //
        // Read-only and collapsed by default; the rules live in
        ...
        Rectangle {
            ...
            Layout.fillWidth: true
            implicitHeight: rulesBody.implicitHeight + 24
            ...
            ColumnLayout {
                ...
                RowLayout {
                    ...
                    ColumnLayout {
                        ...
                        Text {
                            Layout.fillWidth: true
                            text: Tr.t("Read from hypr/modules/windowrules.lua — changing them means editing that file.")
                    ...
                    PillButton {
                        text: rulesCard.expanded ? Tr.t("Hide") : Tr.t("View")
                        icon: rulesCard.expanded ? "󰅃" : "󰅀"
                        implicitWidth: 76
                        implicitHeight: 26
                        active: rulesCard.expanded
                        onClicked: rulesCard.expanded = !rulesCard.expanded
                    }
                }

                Text {
                    Layout.fillWidth: true
                    visible: rulesCard.expanded && CompositorService.windowRules.length === 0
                ...
                Repeater {
                    model: rulesCard.expanded ? CompositorService.windowRules : []
    ...
    ColumnLayout {
        ...
        SettingTiles {
            label: Tr.t("Pace")
            reading: {
                const entry = Theme.motionPresets.find(
                    row => row.scale === SettingsService.motionScale
                        && row.curve === SettingsService.motionCurve)
                return entry ? Tr.t(entry.label) : Tr.t("Custom")
            }
        ...
        SettingTiles {
            label: Tr.t("Preset")
            reading: Tr.t(Motion.labelOf(SettingsService.animationPreset))
